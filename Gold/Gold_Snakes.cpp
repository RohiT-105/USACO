...
//         ll query(ll l,ll r,ll x,ll tl,ll tr)
//         {
//             if(r<tl||tr<l) return LLOONG_MAX;
//             if(l<=tl&&tr<=r) return tree[x];
//             ll mid=(tl+tr)/2;
...
{    
    ...
    fr(i,0,2)
    {
        fr(j,0,c+1)
        {
            fr(k,0,n)
            {
                if(j==0 && i==0 && v[k]>=v[i]) dp[i][j][k]=v[k]-v[i];
    ...
    fr(i,0,n) 
    {
        fr(j,0,c+1) low[i][j]=INT_MAX;
    }
    fr(i,0,n) low[0][0]=min(low[0][0],dp[0][0][i]);
    fr(i,1,n)
    {
        fr(j,0,c+1)
        {
            fr(k,0,n)
            {
                if(v[k]>=v[i]) 
                {
                    if(j>0) dp[1][j][k]=min(dp[0][j][k]+v[k]-v[i],low[i-1][j-1]+v[k]-v[i]);
                    else dp[1][j][k]=dp[0][j][k]+v[k]-v[i];
                }
            }
            fr(k,0,n) low[i][j]=min(low[i][j],dp[1][j][k]);
        }
        memcpy(dp[0],dp[1],sizeof(dp[0]));
        fr(j,0,c+1)
        {
            fr(k,0,n) dp[1][j][k]=INT_MAX;
        }
    }
    ll ans=LLONG_MAX;
    fr(i,0,c+1) ans=min(ans,low[n-1][i]);
    cout<<ans;
}       
signed main()
{ 
