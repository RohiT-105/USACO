    ...
    fr(i,1,n+1)
    {
        fr(j,0,kk+1)
        {
            fr(k,1,4)
            {
                ...
                if(j!=0) 
                {
                    if(k==1) dp[i][j][k]=max({dp[i][j][k],dp[i-1][j-1][2],dp[i-1][j-1][3]});
                    else if(k==2) dp[i][j][k]=max({dp[i][j][k],dp[i-1][j-1][1],dp[i-1][j-1][3]});
                    else dp[i][j][k]=max({dp[i][j][k],dp[i-1][j-1][1],dp[i-1][j-1][2]});
                }
                if(k+1==v[i] || (k==3 && v[i]==1)) dp[i][j][k]++;
            }
        }
    }
    ll ans=0;
