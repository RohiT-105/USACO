...
{    
    ...
    fr(i,1,1001)
    {
        fr(j,1,n+1)
        {
            for(auto x:adj[j]) if(dp[i-1][x]!=LLONG_MIN) dp[i][j]=max(dp[i][j],dp[i-1][x]+val[j]);
        }
    }
    ll ans=0;
    fr(i,0,1001) if(dp[i][1]!=LLONG_MIN) ans=max(ans,dp[i][1]-c*(i*i));
    cout<<ans;
}       
