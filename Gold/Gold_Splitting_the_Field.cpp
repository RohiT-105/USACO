...
// }
ll n;
ll algo(vector<pair<ll,ll>> v)
{
    sort(all(v));
    vll shx(n),slx(n),shy(n),sly(n);
    vll ehx(n),elx(n),ehy(n),ely(n);
    shx[0]=v[0].first;
    slx[0]=v[0].first;
    shy[0]=v[0].second;
    sly[0]=v[0].second;
    ...
    ehx[n-1]=v[n-1].first;
    elx[n-1]=v[n-1].first;
    ehy[n-1]=v[n-1].second;
    ely[n-1]=v[n-1].second;
    rep(i,n-2,0)
    {
        ll x=v[i].first,y=v[i].second;
        ehx[i]=max(ehx[i+1],x);
    ...
    ll max_area=(shx[n-1]-slx[n-1])*(shy[n-1]-sly[n-1]),min_area=LLONG_MAX;
    fr(i,0,n-1)
    {
        ll ar1=(shx[i]-slx[i])*(shy[i]-sly[i]);
        ll ar2=(ehx[i+1]-elx[i+1])*(ehy[i+1]-ely[i+1]);
        min_area=min(min_area,ar1+ar2);
    }
