...
//     return false;
// }
vector<string> mat,visited;
vll dfs(ll row,ll col)
{
    visited[row][col]='#';
    ll area=1;
    ll p=0;
...
void solve()
{
    ...
    mat.pb(s);
    fr(i,0,n+2) visited.pb(s);
    ll ansa=LLONG_MIN,ansp=LLONG_MAX;
    fr(i,1,n+1)
    {
        fr(j,1,n+1)
        {
            if(visited[i][j]=='.' && mat[i][j]=='#')
            {
                ...
                else if(x[1]==ansa) ansp=min(ansp,x[0]); 
            }
        }
    }
    if(ansa!=LLONG_MIN) cout<<ansa<<" "<<ansp;
    else cout<<0<<" "<<0;
}
signed main()
{ 
