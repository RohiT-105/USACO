...
void solve() 
{
    ll n,m;
    cin>>n>>m;
    vll c(m),r(m),d(m),s(m),a(n);
    vector<pair<vector<vll>,ll>> v(n+1);
    fr(i,0,m) 
    {
        cin>>c[i]>>r[i]>>d[i]>>s[i];
        v[c[i]].first.pb({r[i],d[i],s[i]});
    }
    fr(i,0,n) cin>>a[i];
    fr(i,1,n+1) 
    {
    ...
    vll time(n+1,LLONG_MAX);
    time[1]=0;
    priority_queue<vll,vector<vll>,greater<vll>> q;
    q.push({0,1});
    vll port=q.top();
    q.pop();
    fr(i,v[port[1]].second,v[port[1]].first.size())
    // // for(auto x:v[port[1]])
    {
        // if(v[port[1]][i].first[3]==1) continue;
        if(v[port[1]].first[i][0]<port[0]) break;
        v[port[1]].second++;
        if(time[v[port[1]].first[i][1]]>v[port[1]].first[i][2])
        {
            time[v[port[1]].first[i][1]]=v[port[1]].first[i][2];
            q.push({v[port[1]].first[i][2],v[port[1]].first[i][1]});
        }
    }
    while(!q.empty())
    {
        vll port=q.top();
        q.pop();
        if(port[0]!=time[port[1]]) continue;
        // for(auto x:v[port[1]])
        // {
        //     // cout<<port[1]+a[port[0]] <<nl;
        //     if(x[0]>=port[0]+a[port[1]-1] && time[x[1]]>x[2])
        //     {
        //         time[x[1]]=x[2];
        //         q.push({x[2],x[1]});
        //     }
        ...
        {
            // if(v[port[1]][i].first[3]==1) continue;
            if(v[port[1]].first[i][0]<port[0]+a[port[1]-1]) break;
            v[port[1]].second++;
            if(time[v[port[1]].first[i][1]]>v[port[1]].first[i][2])
            {
                time[v[port[1]].first[i][1]]=v[port[1]].first[i][2];
    ...
    fr(i,1,n+1) 
    {
        if(time[i]!=LLONG_MAX) cout<<time[i]<<nl;
        else cout<<-1<<nl;
    }
}
signed main()
