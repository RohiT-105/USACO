...
void solve()
{
    ...
    fr(i,0,n)
    {
        ll cur_f=freq.rbegin()->second,cur_h=freq.rbegin()->first;
        ll x=dat[i][1];
        ll ch;
        if(cows.count(x)) ch=cows[x];
        else 
        {
            cows[x]=g;
            ch=g;
        ...
        ll new_f=freq.rbegin()->second,new_h=freq.rbegin()->first;
        if((new_f!=cur_f || new_h!=cur_h) && !(cur_f==1 && cur_h==ch && new_f==1 && cows[x]==new_h)) ans++;
    }
    cout<<ans;
}
signed main()
