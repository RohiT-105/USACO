...
void solve() 
{
    ...
    // cout<<g;
    if(g==0) cout<<0<<nl<<0<<nl;
    else if(g==1) 
    {
        fr(i,0,gap.size()) if(gap[i]!=0) cout<<min(2ll,gap[i])<<nl<<gap[i];
    }
    else
    {
        // cout<<1;
        fr(i,0,n) ans1=min(ans1,n-(((--upper_bound(all(v),v[i]+n-1)-v.begin()))-i+1));
        if(gap[0]>=gap[n-2]) ans2=accumulate(gap.begin(),--gap.end(),0);
        else ans2=accumulate(++gap.begin(),gap.end(),0);
        cout<<ans1<<nl<<ans2;
    }
}
signed main()
