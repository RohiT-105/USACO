...
{   
    ...
    if(!visited[{0,b,s+1}]) dfs({0,b,s+1});
    if(!visited[{a,0,s+1}]) dfs({a,0,s+1});
    if(!visited[{a,y,s+1}]) dfs({a,y,s+1});
    if(!visited[{x,b,s+1}]) dfs({x,b,s+1});
    ll t1=min((y-b),a);
    ll t2=min((x-a),b);
    if(!visited[{a-t1,b+t1,s+1}]) dfs({a-t1,b+t1,s+1});
    if(!visited[{a+t2,b-t2,s+1}]) dfs({a+t2,b-t2,s+1});
}
void solve()
{
    cin>>x>>y>>k>>m;
