...
void solve()
{
    ...
    for(auto &x:ver) sort(all(x.second));
    for(auto &x:hor) sort(all(x.second));
    pre1=ver;
    pre2=hor;
    for(auto &x:pre1)
    {
        fr(i,1,x.second.size()) x.second[i]+=x.second[i-1]; 
    }
    for(auto &x:pre2)
    {
        fr(i,1,x.second.size()) x.second[i]+=x.second[i-1]; 
    ...
    fr(i,0,n)
    {
        ...
        ll s1=ver[x].size();
        ll s2=hor[y].size();
        ll ind1=lower_bound(all(ver[x]),y)-ver[x].begin();
        ll ind2=lower_bound(all(hor[y]),x)-hor[y].begin();
        ll sum1=(-pre1[x][ind1]+(ind1+1)*(y))+(pre1[x][s1-1]-pre1[x][ind1])-(s1-ind1-1)*y;
        ll sum2=(-pre2[y][ind2]+(ind2+1)*(x))+(pre2[y][s2-1]-pre2[y][ind2])-(s2-ind2-1)*x;
        ans+=(sum1*sum2);
        ans=ans%MOD;
    }
    cout<<ans;
