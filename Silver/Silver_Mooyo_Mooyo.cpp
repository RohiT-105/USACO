...
vector<string> adj;
vector<vll> visited;
ll dfs(ll i,ll j,char c)
{
    if(i<0 || j<0 || i>=n || j>=10) return 0;
    if(adj[i][j]!=c) return 0;
...
void dfs2(ll i,ll j,char c)
{
    if(i<0 || j<0 || i>=n || j>=10) return;
    if(adj[i][j]!=c) return;
    adj[i][j]='0';
    dfs2(i+1,j,c);
    dfs2(i-1,j,c);
