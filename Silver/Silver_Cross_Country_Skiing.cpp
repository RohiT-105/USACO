...
//     return false;
// }
vector<vll> adj,hei,visited;
ll n,m;
ll dfs(ll row,ll col,ll mid)
{
    if(visited[row][col]) return 0;
    visited[row][col]=1;
    ll tot=0;
    if(adj[row][col]) tot++;
    if(row-1>=0 && abs(hei[row][col]-hei[row-1][col])<=mid) tot+=(dfs(row-1,col,mid));
    if(row+1<n && abs(hei[row][col]-hei[row+1][col])<=mid) tot+=(dfs(row+1,col,mid));
    if(col-1>=0 && abs(hei[row][col]-hei[row][col-1])<=mid) tot+=(dfs(row,col-1,mid));  
    if(col+1<m && abs(hei[row][col]-hei[row][col+1])<=mid) tot+=(dfs(row,col+1,mid));
    return tot;
}
void solve()
{
    ...
    hei.resize(n,vll(m,0));
    fr(i,0,n)
    {
        fr(j,0,m) cin>>hei[i][j];
    }
    ll row,col,co=0;
    fr(i,0,n)
    {
