...
void solve()
{
    ...
    fr(i,0,east.size())
    {
        ll ex=east[i].first.second;
        ll ey=east[i].first.first;
        ll ce=east[i].second;
        fr(j,0,north.size())
        {
            ll nx=north[j].first.first;
            ll ny=north[j].first.second;
            ll cn=north[j].second;
            if(stop[ce] || stop[cn]) continue;
