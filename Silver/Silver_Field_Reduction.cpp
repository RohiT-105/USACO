...
void solve() 
{
    ...
    sort(all(x));
    sort(all(y));
    vll p={x[0],x[1],x[2],x[3]};
    vll q={x[n-1],x[n-2],x[n-3],x[n-4]};
    vll r={y[0],y[1],y[2],y[3]};
    vll s={y[n-1],y[n-2],y[n-3],y[n-4]};
    ll ans=LLONG_MAX;
    // fr(i,0,4) cout<<x[i]<<nl;
    fr(i,0,4)
    {
        fr(j,0,4)
        {
            if(q[j]-p[i]<0) continue;
            fr(k,0,4)
            {
                fr(l,0,4)
                {
                    if(s[l]-r[k]<0) continue;
                    ll co=0;
                    fr(o,0,n)
                    {
                        ll x1=p1[o].first,y1=p1[o].second;
                        if(x1>q[j] || x1<p[i] || y1>s[l] || y1<r[k]) co++;
                    }
                    if(co<=3) ans=min(ans,(q[j]-p[i])*(s[l]-r[k]));
                }
            }
        }
    }
