...
void solve() 
{
    ...
    fr(i,0,n)
    {
        fr(j,0,n)
        {
            ...
            m1[v[i][j]]=1;
            ll ch=0;
            map<char,ll> m3,m4,m5;
            fr(k,0,i)
            {
                if(k==i || v[k][j]=='0' || v[k][j]==v[i][j]) continue;
                m3[v[k][j]]=1;
            }
            fr(k,i+1,n)
            {
            ...
            fr(k,0,j)
            {
                if(k==j || v[i][k]=='0' || v[i][k]==v[i][j]) continue;
                if(m3[v[i][k]] || m4[v[i][k]]) ch=1;
                m5[v[i][k]]=1;
            }
            fr(k,j+1,n)
            {
                if(k==j || v[i][k]=='0' || v[i][k]==v[i][j]) continue;
                if(m3[v[i][k]] || m4[v[i][k]] || m5[v[i][k]]) ch=1;
            }
            if(ch) m2[v[i][j]]=1;
        }
    }   
    ll ans=0;
    for(auto x:m1) if(!m2[x.first]) ans++;
    cout<<ans;
}
signed main()
{
