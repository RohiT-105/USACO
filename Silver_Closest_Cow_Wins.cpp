...
void solve()
{
    ...
    fr(i,0,m-1)
    {
        ...
        ll st=p1,end=p1;
        ll dum=st,tot=0;
        while(p1<k && pt[p1].first<e2) p1++;
        fr(j,st,p1) tot+=pt[j].second; 
        ll sum=0,high=0;
        while(st<p1)
        {
            while(end<p1 && 2*(pt[end].first-pt[st].first)<e2-e1)
            {
                sum+=(pt[end].second);
                end++;
            }
    ...
    ll ans=0;
    ll xsx=choices.size();
    fr(i,0,min(xsx,n)) ans+=choices[i];
    cout<<ans;
}
signed main()
{ 
