...
void dfs2(ll node,ll num)
{
    ...
    m[num].insert(node);
    m2[node]=num;
    for(auto x:adj_rev[node]) if(!visited[x]) dfs2(x,num);
}
void solve() 
{
    ll n;
    ...
    fr(i,1,n+1) 
    {
        ...
        fr(j,1,n+1)
        {
            ...
            {
                adj[i].pb(v[i][j]);
                if(i!=v[i][j]) adj_rev[v[i][j]].pb(i);
            }
            if(v[i][j]==i) b=1;
        }
    }
    fr(i,1,n+1) if(!visited[i]) dfs(i);
    fr(i,1,n+1) visited[i]=0;
    while(!s.empty())
    {
        if(!visited[s.top()]) dfs2(s.top(),s.top());
        s.pop();
    }
    fr(i,1,n+1)
    {
