...
void solve() 
{
    ...
    string end;
    getline(cin,end);
    map<string,ll> cow,year;
    map<string,string> zod;
    cow["Bessie"]=0;
    zod["Bessie"]="Ox";
    year={{"Ox",0},{"Tiger",1},{"Rabbit",2},{"Dragon",3},{"Snake",4},
    ...
    {   
        ...
        {
            if(cnt==1) x=word;
            else if(cnt==4) y=word;
            else if(cnt==5) z=word;
            else w=word;
            cnt++;
        }
        zod[x]=z;
        if(y=="previous") 
        {
            if(year[zod[w]]>year[zod[x]]) cow[x]=cow[w]-(year[zod[w]]-year[zod[x]]);
            else cow[x]=cow[w]-(year[zod[w]]+(12-year[zod[x]]));
        }
        else
        {
            if(year[zod[w]]<year[zod[x]]) cow[x]=cow[w]+(year[zod[x]]-year[zod[w]]);
            else cow[x]=cow[w]+((12-year[zod[w]])+(year[zod[x]]));
        }
    }
    cout<<abs(cow["Elsie"]);
}
