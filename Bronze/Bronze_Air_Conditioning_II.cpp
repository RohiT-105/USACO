...
void solve() 
{
    ll n,m;
    cin>>n>>m;
    vll s(n),t(n),c(n),a(m),b(m),p(m),mo(m);
    fr(i,0,n) cin>>s[i]>>t[i]>>c[i];
    fr(i,0,m) cin>>a[i]>>b[i]>>p[i]>>mo[i];
    ll tot=(1<<m);
    ll ans=LLONG_MAX;
    fr(i,0,tot)
    {
        ...
        fr(j,0,n)
        {
            fr(k,s[j],t[j]+1) req[k]+=c[j];
        }
        fr(j,1,101) if(req[j]>row[j]) ch=1;
        if(ch==0) ans=min(ans,cost);
    }
    cout<<ans;
}
