...
vector<pair<ll,ll>> v(1e5+1);
ll ans=LLONG_MAX,x_min,x_max,y_min,y_max;
void dfs(ll node,ll x,ll y)
{
    ...
    y_min=min(y,y_min);
    y_max=max(y,y_max);
    for(auto x:adj[node]) if(!visited[x]) dfs(x,v[x].first,v[x].second);
}
void solve() 
{
    ll n,m;
    ...
    fr(i,1,n+1)
    {
        ...
        if(!visited[i])
        {
            x_min=v[i].first; x_max=v[i].first;
            y_min=v[i].second; y_max=v[i].second;
