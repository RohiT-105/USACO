...
void solve() 
{
    vector<string> s={"Sue", "Buttercup", "Belinda", "Beatrice", "Bella", "Blue", "Betsy","Bessie"};
    ...
    do
    {
        ...
        fr(i,0,s.size())
        {
            if(i-1>=0) ch[s[i]].insert(s[i-1]);
            if(i+1<s.size()) ch[s[i]].insert(s[i+1]);
        }
        ll p=0;
        for(auto x:m)
        {
