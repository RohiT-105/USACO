...
void solve()
{
    ...
    fr(i,0,k)
    {
        ll a,b,c;
        cin>>a>>b>>c;
        ll ind1=lower_bound(all(trees),a)-trees.begin();
        ll ind2=upper_bound(all(trees),b)-trees.begin();
        rest.pb({a,b,(ind2-ind1)-c});
    }
    sort(all(rest));
    ll ans=0,p=0;
    ...
    fr(i,0,n)
    {
        while(p<k && rest[p][0]<=trees[i]) 
        {
            pq.push({ans+rest[p][2],rest[p][1]});
            p++;
        }
        while(!pq.empty() && pq.top().second<trees[i]) pq.pop();
        if(pq.empty()) ans++;
        else if(ans<pq.top().first) ans++;
    }
    cout<<ans<<nl;
}
signed main()
