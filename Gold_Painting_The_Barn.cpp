...
void solve() 
{
    ...
    fr(i,1,202) fr(j,1,202) pre[i][j]+=pre[i-1][j]+pre[i][j-1]-pre[i-1][j-1]; 
    ll ans=0;
    fr(i,1,201) fr(j,1,201) if(pre[i][j]==k) ans++;  
    vector<vll> cost(202,vll(202)); 
    fr(i,1,201)  
    {
        fr(j,1,201)  
        {
            if(pre[i][j]==k) cost[i][j]=-1;
            else if(pre[i][j]==k-1) cost[i][j]=1;
        }
    }
    vll top(202),bottom(202),left(202),right(202);
    ...
    fr(i,1,201)
    {
        ...
        fr(j,i,201)
        {
            fr(k,1,201) row[k]+=cost[j][k];
            ll curr=0;
            fr(k,1,201)
            {
                curr=max(0ll,curr+row[k]);
                top[j]=max(top[j],curr);
            } 
        }
    }
    ...
    fr(i,1,201)
    {
        ...
        fr(j,i,201)
        {
            fr(k,1,201) col[k]+=cost[k][j];
            ll curr=0;
            fr(k,1,201)
            {
                curr=max(0ll,curr+col[k]);
                left[j]=max(left[j],curr);
            } 
        }
    }
