...
void solve() 
{
    ...
    sort(all(v));
    // fr(i,0,n) cout<<v[i]<<nl;  
    fr(i,0,n) v.pb(v[i]+m);
    // fr(i,0,2*n) cout<<v[i]<<nl; 
    // cout<<nl; 
    vll pre(2*n+1);
    fr(i,1,2*n+1) pre[i]=pre[i-1]+v[i-1];
    // fr(i,0,2*n+1) cout<<pre[i]<<nl;
    // cout<<nl;
    if(n%2==0)
    {
        ll ans=LLONG_MAX;
        fr(i,n/2,((n/2)*3)+1) 
        {
            ans=min(ans,(n/2)*v[i]-(pre[i]-pre[i-(n/2)])+ pre[i+(n/2)]-pre[i]-((n/2))*v[i]);
            // cout<<ans<<nl;   
        }
        cout<<ans<<nl;
    }
    else
    {
        ...
        fr(i,n/2,(n/2)*3+2)
        {
            ans=min(ans,((n/2)*v[i]-(pre[i]-pre[i-(n/2)]))+ (pre[i+(n/2)+1]-pre[i+1]-((n/2))*v[i]));
        }
        cout<<ans<<nl;
    }
}
