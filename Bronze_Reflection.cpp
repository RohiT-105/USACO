...
void solve() 
{
    ...
    vector<string> v(n);
    fr(i,0,n) cin>>v[i];
    vector<vll> a(n/2,vector<ll>(n/2,0));
    ll ans=0;
    fr(i,0,n/2)
    {
        fr(j,0,n/2)
        {
            ll x=(n/2)+((n/2)-1)-i,y=(n/2)+((n/2)-1)-j,p=0,q=0;
            if(v[i][j]=='.') p++;
            else q++;
            if(v[i][y]=='.') p++;
            else q++;
    ...
    fr(k,0,r)
    {
        ...
        if(e>=(n/2)) e=(n/2-1)-(e-(n/2));
        if(f>=(n/2)) f=(n/2-1)-(f-(n/2));
        ll i=e,j=f;
        ll x=(n/2)+((n/2)-1)-e,y=(n/2)+((n/2)-1)-f,p=0,q=0;
        if(v[i][j]=='.') p++;
        else q++;
        if(v[i][y]=='.') p++;
        else q++;
