...
string check(ll f,ll g,ll h)
{
    if(f==g && f==h) return "fgh";
    if(f>g && f>h) return "f";
    if(g>f && g>h) return "g";
    if(h>f && h>g) return "h";
    if(f==g) return "fg";
...
void solve() 
{
    ...
    ll f=7,g=7,h=7,ans=0;
    for(auto x:m)
    {
        string a=x.second.first,b=x.second.second;
        string s1=check(f,g,h);
        ll inc=1ll*stoi(b.substr(1));
        if(b[0]=='-') inc=inc*-1;
        if(a=="Bessie") f+=inc;
        else if(a=="Elsie") g+=inc;
        else h+=inc;
        string s2=check(f,g,h);
        if(s1!=s2) ans++;
        // cout<<f<<" "<<g<<" "<<h<<nl;
    }
    cout<<ans;
