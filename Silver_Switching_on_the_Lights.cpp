...
//     return false;
// }
vector<vll> adj,visited;
ll n,m;
map<pair<ll,ll>,set<pair<ll,ll>>> ma;
queue<pair<ll,ll>> q;
void dfs(ll row,ll col)
{
    if(visited[row][col]) return;
    visited[row][col]=1;
    for(auto x:ma[{row,col}]) 
    {
        adj[x.first][x.second]=1;
        q.push({x.first,x.second});
    }
    if(row-1>=0 && adj[row-1][col]==1) dfs(row-1,col);
    if(row+1<n && adj[row+1][col]==1) dfs(row+1,col);
    if(col-1>=0 && adj[row][col-1]==1) dfs(row,col-1);  
    if(col+1<m && adj[row][col+1]==1) dfs(row,col+1);
}
void solve()
{
    cin>>n>>m;
    adj.resize(n,vll(m,0));
    ...
    while(!q.empty())
    {
        ...
        {
            ll p=0;
            if(i-1>=0 && visited[i-1][j]==1) p=1;
            if(j-1>=0 && visited[i][j-1]==1) p=1;
            if(j+1<m && visited[i][j+1]==1) p=1;
            if(i+1<n && visited[i+1][j]==1) p=1;
            if(p) dfs(i,j);
        }
    }
    fr(i,0,n)
    {
        fr(j,0,m) if(adj[i][j]==1) ans++;
    }
    cout<<ans;
}   
signed main()
